...
export default function SignUp() {
  ...
  const formSchema = z.object({
    ...
    password: z.string().regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, {message: 'Password should contain 1 special character, 1 number ,1 uppercase letter, 1 lowercase letter and min 8 characters of length.'}),
    isWebAuthnReg: z.boolean({message: 'Tick to register with Web Authn'}),
    name: z.string().min(2, { message: 'Name should contain atleast 2 characters'}).max(20, {message: 'Name should not exceed more than 20 characters'})
...
const onSubmit = async (data) => {
        try{
          ...
          const verifyRegResponse =  await verifyRegistration({attResp, id: responseData.id});
  ...
  return (
    <ThemeProvider theme={defaultTheme}>
      ...
      <SimpleSnackbar handleSnackbar={handleSnackbar} toggleSnackbar={toggleSnackbar} snackbarMessage={snackbarMessage} />
    </ThemeProvider>
  );
}
